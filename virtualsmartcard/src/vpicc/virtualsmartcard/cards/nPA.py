...
class nPA_SAM(SAM):
    ...
    def __eac_abort(self):
        pace.EAC_CTX_clear_free(self.current_SE.eac_ctx)
    ...
    def __eac_pace_step1(self, data):
        ...
        self.current_SE.eac_ctx = pace.EAC_CTX_new()
        ...
        pace.EAC_CTX_init_ef_cardaccess(ef_card_access_data, self.current_SE.eac_ctx)

        nonce = pace.buf2string(pace.PACE_STEP1_enc_nonce(self.current_SE.eac_ctx, self.sec))
    ...
    def __eac_pace_step2(self, data):
        ...
        pubkey = pace.buf2string(pace.PACE_STEP3A_generate_mapping_data(self.current_SE.eac_ctx))

        for tag, length, value in tlv_data:
            if tag == 0x81:
                pace.PACE_STEP3A_map_generator(self.current_SE.eac_ctx, pace.get_buf(value))
            else:
                raise SwError(SW["ERR_INCORRECTPARAMETERS"])
    ...
    def __eac_pace_step3(self, data):
        ...
        my_epp_pubkey = pace.buf2string(pace.PACE_STEP3B_generate_ephemeral_key(self.current_SE.eac_ctx))

        for tag, length, value in tlv_data:
            if tag == 0x83:
                self.pace_opp_pub_key = pace.get_buf(value)
                pace.PACE_STEP3B_compute_shared_secret(self.current_SE.eac_ctx, self.pace_opp_pub_key)
            else:
                raise SwError(SW["ERR_INCORRECTPARAMETERS"])
    ...
    def __eac_pace_step4(self, data):
        ...
        pace.PACE_STEP3C_derive_keys(self.current_SE.eac_ctx)
        my_token = pace.buf2string(pace.PACE_STEP3D_compute_authentication_token(self.current_SE.eac_ctx, self.pace_opp_pub_key))

        for tag, length, value in tlv_data:
            if tag == 0x85:
                pace.PACE_STEP3D_verify_authentication_token(self.current_SE.eac_ctx, pace.get_buf(value))
            else:
                raise SwError(SW["ERR_INCORRECTPARAMETERS"])
